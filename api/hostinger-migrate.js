...
export default async function handler(req, res) {
  ...
  try {
    const { audioUrl, fileKey } = req.body;

    if (!audioUrl || !fileKey) {
      return res.status(400).json({ error: 'Missing audioUrl or fileKey' });
    }

    // Download from R2/external URL
    let audioBuffer;
    try {
      ...
      return res.status(500).json({ error: `Download failed: ${fetchError.message}` });
    }
    ...
    console.error('Migrate handler error:', error);
