export default async function handler(req, res) {
  ...
  try {
    const testUrl = 'https://audio-secondary.alexbrin102.workers.dev/health-test';
    ...
    if (response.ok || response.status === 404 || response.status === 206) {
      res.status(200).json({ 
        status: 'healthy', 
        message: 'Secondary proxy is working',
        timestamp: new Date().toISOString()
      });
    } else {
      res.status(503).json({ 
        status: 'unhealthy', 
        message: `Secondary proxy returned status ${response.status}`,
        timestamp: new Date().toISOString()
      });
    }
  } catch (error) {
    res.status(503).json({ 
      status: 'unhealthy', 
      message: `Secondary proxy error: ${error.message}`,
      timestamp: new Date().toISOString()
    });
  }
} 