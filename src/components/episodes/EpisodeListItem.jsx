...
import EpisodeCacheControls from './EpisodeCacheControls';
...
const EpisodeListItem = React.memo(({ episode, currentLanguage, questionsCount, questionsForEpisode }) => {
  ...
  return (
    <li className="bg-slate-800/70 rounded-xl shadow-lg hover:shadow-purple-500/20 transition-all duration-300 ease-in-out border border-slate-700/50 hover:border-purple-500/60 animate-fade-in-up relative">
      <div className="p-4">
        {/* Кнопка кеширования в правом верхнем углу */}
        <div className="absolute top-3 right-3">
          <EpisodeCacheControls
            episode={episode}
            currentLanguage={currentLanguage}
          />
        </div>
        
        <Link to={`/episode/${episode.slug}`} className="block group mb-2 pr-16">
